// Code generated by go-swagger; DO NOT EDIT.

package status_update_templates

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"
	"github.com/go-openapi/strfmt"

	"github.com/firehydrant/api-client-go/models"
)

// GetV1StatusUpdateTemplatesStatusUpdateTemplateIDReader is a Reader for the GetV1StatusUpdateTemplatesStatusUpdateTemplateID structure.
type GetV1StatusUpdateTemplatesStatusUpdateTemplateIDReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *GetV1StatusUpdateTemplatesStatusUpdateTemplateIDReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {
	case 200:
		result := NewGetV1StatusUpdateTemplatesStatusUpdateTemplateIDOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil
	default:
		return nil, runtime.NewAPIError("[GET /v1/status_update_templates/{status_update_template_id}] getV1StatusUpdateTemplatesStatusUpdateTemplateId", response, response.Code())
	}
}

// NewGetV1StatusUpdateTemplatesStatusUpdateTemplateIDOK creates a GetV1StatusUpdateTemplatesStatusUpdateTemplateIDOK with default headers values
func NewGetV1StatusUpdateTemplatesStatusUpdateTemplateIDOK() *GetV1StatusUpdateTemplatesStatusUpdateTemplateIDOK {
	return &GetV1StatusUpdateTemplatesStatusUpdateTemplateIDOK{}
}

/*
GetV1StatusUpdateTemplatesStatusUpdateTemplateIDOK describes a response with status code 200, with default header values.

Get a single status update template by ID
*/
type GetV1StatusUpdateTemplatesStatusUpdateTemplateIDOK struct {
	Payload *models.StatusUpdateTemplateEntity
}

// IsSuccess returns true when this get v1 status update templates status update template Id o k response has a 2xx status code
func (o *GetV1StatusUpdateTemplatesStatusUpdateTemplateIDOK) IsSuccess() bool {
	return true
}

// IsRedirect returns true when this get v1 status update templates status update template Id o k response has a 3xx status code
func (o *GetV1StatusUpdateTemplatesStatusUpdateTemplateIDOK) IsRedirect() bool {
	return false
}

// IsClientError returns true when this get v1 status update templates status update template Id o k response has a 4xx status code
func (o *GetV1StatusUpdateTemplatesStatusUpdateTemplateIDOK) IsClientError() bool {
	return false
}

// IsServerError returns true when this get v1 status update templates status update template Id o k response has a 5xx status code
func (o *GetV1StatusUpdateTemplatesStatusUpdateTemplateIDOK) IsServerError() bool {
	return false
}

// IsCode returns true when this get v1 status update templates status update template Id o k response a status code equal to that given
func (o *GetV1StatusUpdateTemplatesStatusUpdateTemplateIDOK) IsCode(code int) bool {
	return code == 200
}

// Code gets the status code for the get v1 status update templates status update template Id o k response
func (o *GetV1StatusUpdateTemplatesStatusUpdateTemplateIDOK) Code() int {
	return 200
}

func (o *GetV1StatusUpdateTemplatesStatusUpdateTemplateIDOK) Error() string {
	return fmt.Sprintf("[GET /v1/status_update_templates/{status_update_template_id}][%d] getV1StatusUpdateTemplatesStatusUpdateTemplateIdOK  %+v", 200, o.Payload)
}

func (o *GetV1StatusUpdateTemplatesStatusUpdateTemplateIDOK) String() string {
	return fmt.Sprintf("[GET /v1/status_update_templates/{status_update_template_id}][%d] getV1StatusUpdateTemplatesStatusUpdateTemplateIdOK  %+v", 200, o.Payload)
}

func (o *GetV1StatusUpdateTemplatesStatusUpdateTemplateIDOK) GetPayload() *models.StatusUpdateTemplateEntity {
	return o.Payload
}

func (o *GetV1StatusUpdateTemplatesStatusUpdateTemplateIDOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.StatusUpdateTemplateEntity)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
